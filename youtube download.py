...
def download():
    ...
    if check.get() == 2:
        ...
        messagebox.showinfo(
            'Success', 'Complete Playlist is downloaded succesfully')
        os.startfile(got_path)
...
root.title('Youtube Video Downloader by Arbaz Ali')
root.config(bg='red4')
...
innerframe = LabelFrame(outerframe, text='DOWNLOAD',
...
text = StringVar()
url_entryField = Entry(outerframe, width=60, font=(
    'arial', 14, 'bold'), justify=CENTER, textvariable=text, fg='gray')
...
downloadButton = Button(outerframe, image=downloadImage,
                        bg='red4', command=download)
